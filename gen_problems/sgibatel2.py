...
# from gen_origami import fold_all_facets_by_line, mkpt
...
def bend(pts, facets):
  ...
  while len(to_process) > 0:
    if DRAW:
      draw_side_by_side([
        [pts[p] for p in f] for f in facets],
        [transform_facet(f, processed[f]) for f in processed])
...
def get_cat_facets():
    ...
    mats = bend(pts, facets)
    return pts, facets, mats
