...
parser.add_argument('--rot_triplet', type=str, default=None)
args = parser.parse_args()

if args.rot_triplet is not None:
  rot = [int(x) for x in args.rot_triplet.split(' ')]
else:
  rot = rots[ args.rot ]

print >> sys.stderr, " ... using rotation:", rot 
...
mat_init_fit = (mat_rot**-1) * mat_shift
...
while fx < 1:
  ...
  # if too long:
  if sol_len(serialize_solution([(f[0], mat_init_fit * f[1]) for f in facet_pairs_cand])) > 5000:
    print >> sys.stderr, "Size exceeded limit"
    pts = pts_backup
    break
...
facet_pairs = [(f[0], mat_init_fit * f[1]) for f in facet_pairs]
